...
def test_create_role(client: TestClient, admin_user: User):
...
def test_create_role_already_exists(
    client: TestClient, admin_user: User, user_role: Role
):
    token = make_login(client, admin_user.email, admin_user.clean_password)
    response = client.post(
        "/role",
...
def test_get_many_roles(
    client: TestClient, admin_user: User, user_role: Role, admin_role: Role
):
    token = make_login(client, admin_user.email, admin_user.clean_password)
    response = client.get(
        "/role",
...
def test_get_role_by_id(client: TestClient, admin_user: User, user_role: Role):
    token = make_login(client, admin_user.email, admin_user.clean_password)
    response = client.get(
        f"/role/{user_role.id}",
        headers={"Authorization": f"Bearer {token}"},
...
def test_get_role_by_id_not_exists(client: TestClient, admin_user: User):
    token = make_login(client, admin_user.email, admin_user.clean_password)
    response = client.get(
        f"/role/999",
        headers={"Authorization": f"Bearer {token}"},
...
def test_delete_role(client: TestClient, admin_user: User, user_role: Role):
    token = make_login(client, admin_user.email, admin_user.clean_password)
...
def test_delete_role_not_exists(client: TestClient, admin_user: User):
    token = make_login(client, admin_user.email, admin_user.clean_password)
    response = client.delete(
        f"/role/999",
        headers={"Authorization": f"Bearer {token}"},
...
def test_update_role(client: TestClient, admin_user: User, user_role: Role):
    token = make_login(client, admin_user.email, admin_user.clean_password)
...
def test_update_role_not_exists(client: TestClient, admin_user: User):
    token = make_login(client, admin_user.email, admin_user.clean_password)
    response = client.put(
        "/role/9999",
        headers={"Authorization": f"Bearer {token}"},
